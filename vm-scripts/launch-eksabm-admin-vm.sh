...
if [ $? -eq 0 ]
then
    exit_out "ERROR:: Detected an error...${EKSA_ADMIN_IP_ONLY} is already used elsewhere as per ${GLOBAL_ALLOCATION_TABLE}. PLease provide another IP"
else
...
[ -d ${EKSA_ADMIN_DIR} ] && exit_out "ERROR:: Detected an error...${EKSA_ADMIN_DIR} exists. Exiting"
