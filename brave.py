...
def execute_remote_command_shell(hostname, port, username, password, command, ssh_private_key_path=None):
    ...
    try:
        ...
        while True:
            ...
            if ('Cluster created' in output) or ('Traceback' in output) or ('ERROR::' in output) or (':EXITING:' in output) or ('vms launched.' in output) or ('ClusterHealthy has progressed to status Success' in output):
                print(output, end='')
                ssh_session.close()
                client.close()
                break
